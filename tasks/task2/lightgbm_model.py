...
def train_with_tuning(X_train, y_train, X_val, y_val):
    ...
    print(f"  Best iteration: {best.best_iteration_}")
    return best
...
def _plot_residuals(y_test, y_pred, plot_dir):
    ...
    axes[0].scatter(y_test, y_pred, alpha=0.3, s=5, color="#e67e22")
    ...
    plt.suptitle("LightGBM - Model Evaluation", fontsize=13, fontweight="bold")
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, "lgbm_residuals.png"), dpi=150)
    plt.close()
...
def _plot_feature_importances(model, plot_dir):
    ...
    ax.bar(sorted_names, sorted_vals, color="#e67e22", edgecolor="white")
    ax.set_title("Feature Importances - LightGBM", fontweight="bold")
    ax.set_ylabel("Importance")
    ax.set_xlabel("Feature")
    plt.xticks(rotation=30, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, "lgbm_feature_importances.png"), dpi=150)
    plt.close()
...
if __name__ == "__main__":
    ...
    print("\n--- Saving ---")
    save_model(model, os.path.join(SAVE_DIR, "lightgbm.pkl"))
